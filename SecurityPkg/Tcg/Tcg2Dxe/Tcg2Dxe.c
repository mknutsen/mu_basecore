  ...
  )
{
  ...
  if (CompareGuid (PcdGetPtr(PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceNoneGuid) ||
    ...
    LogCriticalTelemetryString(MS_RSC_TPM_INIT_FAILED, 0xA, CompareGuid (PcdGetPtr(PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceNoneGuid), "PalindromeUefi.Tpm.InitFail");
    return EFI_UNSUPPORTED;
  }

  if (GetFirstGuidHob (&gTpmErrorHobGuid) != NULL) {
    DEBUG ((EFI_D_ERROR, "TPM2 error!\n"));
    LogCriticalTelemetryString(MS_RSC_TPM_INIT_FAILED, 0xB, 0, "PalindromeUefi.Tpm.InitFail");
    return EFI_DEVICE_ERROR;
  }
  ...
  if (EFI_ERROR (Status)) {
    ...
    LogCriticalTelemetryString(MS_RSC_TPM_INIT_FAILED, 0xC, Status, "PalindromeUefi.Tpm.InitFail");
    return Status;
  }
